...
func (self EmailLogic) EmailNotice() {
	...
	for {
		...
		cursor, findErr := db.GetCollection("user").Find(ctx, bson.M{"_id": bson.M{"$gt": lastUid}}, opts)
		if findErr != nil {
			logger.Errorln("find user error:", findErr)
			continue
		}
		err = cursor.All(ctx, &users)
		cursor.Close(ctx)
		if err != nil {
			logger.Errorln("find user cursor error:", err)
			continue
		}
