...
func (self MissionLogic) RedeemLoginAward(ctx context.Context, me *model.Me) error {
	...
	_, err = session.WithTransaction(ctx, func(sessCtx mongo.SessionContext) (interface{}, error) {
		if userLoginMission.Uid == 0 {
			...
			_, txErr := db.GetCollection("user_login_mission").InsertOne(sessCtx, userLoginMission)
			if txErr != nil {
				objLog.Errorln("insert user_login_mission error:", txErr)
			...
			_, txErr := db.GetCollection("user_login_mission").ReplaceOne(sessCtx,
				bson.M{"_id": userLoginMission.Uid}, userLoginMission)
			if txErr != nil {
				objLog.Errorln("update user_login_mission error:", txErr)
				return nil, errors.New("服务内部错误")
			}
		}

		desc := times.Format("Ymd") + " 的每日登录奖励 " + strconv.Itoa(userLoginMission.Award) + " 铜币"
		txErr := self.changeUserBalance(sessCtx, me, model.MissionTypeLogin, userLoginMission.Award, desc)
		if txErr != nil {
			objLog.Errorln("changeUserBalance error:", txErr)
			return nil, errors.New("服务内部错误")
		}
...
func (MissionLogic) FindLoginMission(ctx context.Context, me *model.Me) *model.UserLoginMission {
	...
	err := db.GetCollection("user_login_mission").FindOne(ctx, bson.M{"_id": me.Uid}).Decode(userLoginMission)
	if err != nil {
...
func (MissionLogic) Complete(ctx context.Context, me *model.Me, id string) error {
	...
	err := db.GetCollection("mission").FindOne(ctx, bson.M{"_id": goutils.MustInt(id)}).Decode(mission)
	if err != nil {
		objLog.Errorln("MissionLogic FindLoginMission error:", err)
		return err
	}
...
func (MissionLogic) findMission(ctx context.Context, typ int) *model.Mission {
	mission := &model.Mission{}
	db.GetCollection("mission").FindOne(ctx, bson.M{"type": typ}).Decode(mission)
	return mission
}

func (self MissionLogic) changeUserBalance(ctx context.Context, me *model.Me, typ, award int, desc string) error {
	_, err := db.GetCollection("user_info").UpdateOne(ctx,
		bson.M{"_id": me.Uid},
		bson.M{"$inc": bson.M{"balance": award}})
	if err != nil {
		return errors.New("服务内部错误")
	}
	...
	return DefaultUserRich.add(ctx, balanceDetail)
}
