...
func (self AutoCrawlLogic) DoCrawl(isAll bool) error {
	...
	cursor, err := db.GetCollection("auto_crawl_rule").Find(ctx, bson.M{"status": model.AutoCrawlOn})
	if err != nil {
		logger.Errorln("AutoCrawlLogic FindBy Error:", err)
		return err
	}
	defer cursor.Close(ctx)
	if err = cursor.All(ctx, &autoCrawlConfList); err != nil {
		logger.Errorln("AutoCrawlLogic cursor Error:", err)
		return err
	}
...
func (self AutoCrawlLogic) CrawlWebsite(website string, isAll bool) error {
	...
	err := db.GetCollection("auto_crawl_rule").FindOne(ctx, bson.M{"website": website}).Decode(autoCrawlConf)
	if err != nil {
