...
func (self RankLogic) GenDayRank(objtype, objid, num int) {
	if objtype == model.TypeTopic {
		...
		topic := &model.Topic{}
		err := db.GetCollection("topics").FindOne(ctx, bson.M{"_id": objid}).Decode(topic)
		if err != nil {
			return
		}

		topicNode := &model.TopicNode{}
		err = db.GetCollection("topics_node").FindOne(ctx, bson.M{"_id": topic.Nid}).Decode(topicNode)
		if err != nil {
			return
		}
...
func (self RankLogic) FindRichRank(ctx context.Context) []*model.User {
	...
	cursor, err := db.GetCollection("user_info").Find(ctx,
		bson.M{"balance": bson.M{"$gt": 0}},
		options.Find().SetSort(bson.M{"balance": -1}).SetLimit(25))
	if err != nil {
	...
	if err = cursor.All(ctx, &userList); err != nil {
		objLog.Errorln("find rich rank decode error:", err)
		return nil
	}
