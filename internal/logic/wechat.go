...
func (self WechatLogic) CheckSession(ctx context.Context, code string) (*model.WechatUser, error) {
	...
	err = db.GetCollection("wechat_user").FindOne(ctx, bson.M{"openid": openid}).Decode(wechatUser)
	...
	if wechatUser.Id == 0 {
		...
		var id int
		id, err = db.NextID("wechat_user")
		if err != nil {
			objLog.Errorln("WechatLogic WxLogin NextID error:", err)
			return nil, err
		}
		wechatUser.Id = id
		_, err = db.GetCollection("wechat_user").InsertOne(ctx, wechatUser)
		if err != nil {
...
func (self WechatLogic) Bind(ctx context.Context, id, uid int, userInfo string) (*model.WechatUser, error) {
	...
	_, err := db.GetCollection("wechat_user").UpdateOne(ctx, bson.M{"_id": id}, bson.M{"$set": wechatUser})
	if err != nil {
...
func (self WechatLogic) AutoReply(ctx context.Context, reqData []byte) (*model.WechatReply, error) {
	...
	case model.WeMsgTypeText:
		if strings.Contains(wechatMsg.Content, "晨读") {
			...
			db.GetCollection("wechat_auto_reply").FindOne(ctx, bson.M{"word": bson.M{"$regex": wechatMsg.Content}}).Decode(autoReply)
			if autoReply.Id != 0 {
	...
	case model.WeMsgTypeEvent:
		...
		case model.WeEventSubscribe:
			...
			autoReply := &model.WechatAutoReply{}
			err = db.GetCollection("wechat_auto_reply").FindOne(ctx, bson.M{"typ": model.AutoReplyTypSubscribe}).Decode(autoReply)
			if err == nil {
...
func (self WechatLogic) CheckCaptchaAndActivate(ctx context.Context, me *model.Me, captcha string) error {
	...
	_, err = session.WithTransaction(ctx, func(sc mongo.SessionContext) (interface{}, error) {
		_, txErr := db.GetCollection("wechat_user").UpdateOne(sc, bson.M{"openid": openid}, bson.M{"$set": bson.M{"uid": me.Uid}})
		if txErr != nil {
			return nil, txErr
		}

		_, txErr = db.GetCollection("user").UpdateOne(sc, bson.M{"_id": me.Uid}, bson.M{"$set": bson.M{
...
func (self WechatLogic) checkAndSave(ctx context.Context, wechatMsg *model.WechatMsg) error {
	...
	wechatUser := &model.WechatUser{}
	err = db.GetCollection("wechat_user").FindOne(ctx, bson.M{"openid": wechatMsg.FromUserName}).Decode(wechatUser)
	if err != nil && err != mongo.ErrNoDocuments {
		return err
	}
	...
	if wechatUser.Openid != "" {
		...
		_, err = db.GetCollection("wechat_user").UpdateOne(ctx, bson.M{"_id": wechatUser.Id}, bson.M{"$set": wechatUser})
	} else {
		wechatUser = &model.WechatUser{
