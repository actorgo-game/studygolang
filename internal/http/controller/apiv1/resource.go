...
func (ResourceController) Delete(ctx echo.Context) error {
	meVal := me(ctx)
	if meVal.Uid == 0 {
		return fail(ctx, "请先登录")
	}
	id := goutils.MustInt(ctx.FormValue("id"))
	err := logic.DefaultResource.Delete(context.EchoContext(ctx), id, meVal.Uid, meVal.IsRoot)
	if err != nil {
